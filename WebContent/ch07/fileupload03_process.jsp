                    ...
                    <div class="content-left wow fadeInLeft" data-wow-delay=".3s">
						...
						<%
							...
							if(!tempFile.exists()){ //설정한 경로에 폴더가 없으면
								tempFile.mkdirs();	//경로에 있는 모든 폴더를 만들어줘
							}
							
							//Commons-FileUpload를 이용하여 파일을 업로드하려면 먼저
							//Commons-FileUpload라이브러리를 활용
							//파일 업로드를 위해 패키지에 포함되어 있는 DiskFileUpload 객체 생성
							...
							while(params.hasNext()){
								...
								//일반 데이터인지 파일인지 분석 및 처리하여 파일을 업로드함
								if(!fileItem.isFormField()){ //일반데이터가 아니면 == 파일이면
